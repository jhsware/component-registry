'use strict';
var development = typeof process !== 'undefined' && process.env.NODE_ENV !== 'production';
    ...
    */
    ...
    if (name) {
        ...
        if (utilities.namedUtility[name]) {
            // Utility already registered, throw RegistrationError
            throw new UtilityRegistryException("There is a utility already registered for: (" + implementsInterface._name + ", '" + name + "'). Check that you have registered it!")
        }
        ...
        if (utilities.unnamedUtility) {
            ...
            throw new UtilityRegistryException("There is a utility already registered for: (" + implementsInterface._name + "'). Check that you have registered it!")
        }
        ...
        if (utilities.namedUtility[name]) {
            ...
            if (arguments.length === 3) {
                ...
                if (development) {
                    ...
                    Object.keys(utilities.namedUtility).forEach(function (key) {
                        var util = this.utilities[key]
                        message.push("[" + util.implements._name + "." + key + "] " + util.implements.interfaceId);
                    });
                }
        ...
        if (arguments.length === 3) {
            ...
            if (development) {
                ...
                Object.keys(this.utilities).forEach(function (key) {
                    var util = this.utilities[key]
                    message.push("[" + util.implements._name + "] " + util.implements.interfaceId);
                });
